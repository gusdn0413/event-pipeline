...
| 파일 | 포함 리소스 | 대상 |
|---|---|---|
| `docs/k8s/configmap.yaml` | ConfigMap | 앱 환경 설정 (NATS_URL, 외부 DB 주소 등) |
| `docs/k8s/secret.yaml` | Secret | DB password |
| `docs/k8s/nats.yaml` | Deployment + Service | NATS 브로커 |
| `docs/k8s/app.yaml` | Deployment + Service | 이벤트 생성기 앱 (replicas 3) |
| `docs/k8s/grafana.yaml` | Deployment + Service | Grafana 시각화 대시보드 |
| `docs/k8s/grafana-pvc.yaml` | PersistentVolumeClaim | Grafana 대시보드·사용자 설정 영구 저장 |
...
*   **역할**: Grafana 컨테이너 1대와, 외부 접근용 LoadBalancer Service, 그리고 대시보드 설정을 저장할 영구 디스크(PVC)
*   **선택 이유**:
